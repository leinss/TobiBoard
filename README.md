...
TurtleBoard keeps everything HeliBoard does well and layers on a voice-input pipeline, tighter security defaults, and smoother settings. In short:

|                                | HeliBoard | TurtleBoard |
| ------------------------------ | :-------: | :---------: |
| AOSP / OpenBoard core          |     ✅     |      ✅      |
...
- **Side-by-side install.** TurtleBoard uses its own application ID (`helium314.keyboard.turtleboard`) so it sits next to HeliBoard instead of replacing it — easy to try, easy to back out.

## Voice privacy

Voice input is opt-in and disabled by default. When you enable it:

- Your OpenRouter API key is stored in `EncryptedSharedPreferences` (AES-256-GCM, key material backed by the Android Keystore on API 23+). It is excluded from both Android cloud backup and device-to-device transfer, is filtered out of TurtleBoard's own backup/restore ZIP, and is never written to logs.
...
1. Install from [Releases](https://github.com/Turtlecute33/TurtleBoard/releases/latest). Each release ships `SHA256SUMS.txt` — run `sha256sum -c SHA256SUMS.txt` to verify the APK before installing.
2. Enable TurtleBoard in **Settings > System > Keyboard**.
3. (Optional) For voice input:
   1. Create an API key at [openrouter.ai/keys](https://openrouter.ai/keys).
   2. Open **TurtleBoard Settings > Voice Input**, paste the key, and enable **Voice Input**. You'll get a microphone-access rationale dialog before the system permission prompt.
...
All of HeliBoard's functionality is preserved unchanged — TurtleBoard does not rip anything out:
...
- Backup and restore settings (with sensitive keys filtered out on TurtleBoard)
